...
train_dir = '/kaggle/input/3-class/3 class/train'
val_dir = '/kaggle/input/3-class/3 class/validation'

img_width, img_height = 224, 224

model3_54m_2 = tf.keras.models.Sequential([
...
model3_54m_2.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

epoch = 50
history = model3_54m_2.fit(train_generator, epochs=epoch, validation_data=val_generator)
...
test_data_dir = '/kaggle/input/3-class/3 class/test'
...
test_loss, test_accuracy = model3_54m_2.evaluate(test_generator)

print(f'Test loss: {test_loss:.4f}')
print(f'Test accuracy: {test_accuracy:.4f}')

# Predict class probabilities for each sample in the test set
predictions = model3_54m_2.predict(test_generator)
...
plt.savefig('/kaggle/working/confusion_matrix for model3_54m_2.png')
...
acc_3C.to_csv("/kaggle/working/accuracy of train vs validation for model3_54m_2.csv")
loss_3C.to_csv("/kaggle/working/loss of train vs validation for model3_54m_2.csv")
